                                ...
                                withCredentials([string(credentialsId: 'SONAR_TOKEN', variable: 'SONAR_TOKEN')]) {
                                    if (currentSvc in ["backoffice", "storefront"]) {
                                        ...
                                        sh """
                                            ...
                                            mvn sonar:sonar -X -Dsonar.projectKey=intro-to-devops_yas-${currentSvc} -Dsonar.organization=intro-to-devops -Dsonar.host.url=https://sonarcloud.io -Dsonar.token=\$SONAR_TOKEN -Dsonar.ws.timeout=300 -Dsonar.working.directory=${env.WORKSPACE}/${currentSvc}/target/sonar-scanner
                                        """
                                    }
                                }
                            }
